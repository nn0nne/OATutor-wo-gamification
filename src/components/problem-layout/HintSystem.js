...
class HintSystem extends React.Component {
    ...
    render() {
        ...
        console.log("HintSystem - all hints:", hints);
        hints.forEach((hint, i) => {
            console.log(`Hint ${i}:`, {
                showCode: hint.showCode,
                code: hint.code,
                codeExists: !!hint.code,
                codeTrimmed: hint.code ? hint.code.trim() : null,
                shouldShow: hint.showCode && hint.code && hint.code.trim()
            });
        });
                    ...
                    >
                        ...
                        <AccordionDetails >
                            <div style={{ width: "100%" }}>
                                ...
                                {/* Display code for top-level hint - ONLY if hint is unlocked */}
                                {hint.showCode && hint.code && hint.code.trim() && this.props.hintStatus[i] && (
                                    <pre style={{
                                ...
                                {/* Display code for subhints when they're shown - ONLY if subhint is unlocked */}
                                {(showSubHints[i] || (use_expanded_view && debug)) &&
                                    hint.subHints && hint.subHints.map((subHint, subIndex) => (
                                        this.state.subHintsFinished[i][subIndex] &&
                                        subHint.showCode && subHint.code && subHint.code.trim() && (
                                            <div key={subHint.id}>
                                                <pre style={{
